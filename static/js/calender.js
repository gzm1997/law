(function() {
    ...
    Calendar = (function() {
        ...
        Calendar.prototype.drawDays = function() {
            ...
            //就这里的ran数组先用随机代替，接下来将收到server的数据填到这个ran数组就可以了

            var ran = new Array();
            for (i = _j = 1; _j <= lastDayOfMonth; i = _j += 1) {
                ran[i] = Math.floor(15 * Math.random(10));
                this.$days.append($("<li class='drp-day " + (this.dayClass(i, firstDayOfMonth, lastDayOfMonth)) + "'><span class='unread'><p class='msg-num'>" + ran[i] + "</p></span><p class='which-day'>" + i + "</p></li>"));
            }
            return this.$calendar.find('.drp-day').click(function(evt) {
                ...
                day = parseInt($(this).children(".which-day").text(), 10);
                console.log(this);
                if (isNaN(day)) {
                    //console.log(233);
                    return false;
